...
int LDS:: Storage_init(const std::string& storage_path){
	//after the init, we have written the inital version in the version log, and the online-map
	//1.get the storage properties: blk64 in bytes. We then caculate the slot number. The backup log is 4 times size of the slot. The version log is just set to be 64MB (we may adjust it by the slot number of slot size).

	//char *mmaped_dev;
	//2. this function only fills the basic structure, such as pointers for accessing version log and backup log, inital version is filled by the recovery process in LevelDB. that is, if flash_using_exist is 0, leveldb will create a new DB in which it writes the inital version to version log.
	//manifest= new LDS_Log;
	//backup = new LDS_Log;

	//manifest->addr=mmaped_dev;
	//manifest->size= VERSION_LOG_SIZE;
	//manifest->write_head=0;

	//backup->addr= mmaped_dev+VERSION_LOG_SIZE;
	//backup->size= SLOT_SIZE *4;
	//backup->write_head=0;
...
int LDS:: LDS_recover(const std::string& storage_path){
	//1. this function only fills the basic structures, such as pointers for accessing version log and backup log. the online-map is filled by the actual recovery process in leveldb.
